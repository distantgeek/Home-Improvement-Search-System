...
- **Event search:** Google search results via [Serper.dev](https://serper.dev).
...
- **City → County enrichment:** Derived from Census ZCTA-to-Place + ZCTA-to-County join. Also regenerated by the build script.
...
data/zip-county.json    # ZIP → county lookup (~3,940 entries, generated from Census)
data/city-county.json   # City → county lookup (~3,822 entries, generated from Census)
.github/workflows/      # GitHub Actions — builds and pushes to GHCR on push to main
```
...
git add data/zip-county.json data/city-county.json
git commit -m "Regenerate Census ZIP→county and city→county lookups"
```

Requires `bash`, `curl`, `awk`, and `python3`. Downloads two files from the Census Bureau
(~10 MB total) and emits:
- `data/zip-county.json` — ~3,940 ZIP entries across VA, MD, PA, DC, NJ, DE
- `data/city-county.json` — ~3,822 city entries (ZCTA-to-Place joined with ZCTA-to-County)
