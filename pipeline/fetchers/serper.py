...
def build_queries_for_state(
    ...
    """Generate Serper search query strings for one state (current and next year)."""
...
def _extract_state_from_query(query: str) -> str | None:
    ...
    Sorted longest-first so "West Virginia" matches before "Virginia".
    """
    lower = query.lower()
    for code in sorted(STATE_ORDER, key=lambda c: len(STATE_NAMES[c]), reverse=True):
